...
const SUGGESTED_PROMPTS = [
  ...
  "Can fund equipment and staff training in Chhattisgarh",
]
...
export function ChatPanel({
  ...
  const renderMessageContent = (content: string) => {
    ...
    return parts.map((part, index) => {
      if (testRegex.test(part)) {
        ...
        return (
          <a key={index} href={`tel:${cleanPhone}`} className="text-blue-600 underline font-medium">
            {part}
          </a>
        )
      }
  ...
  // Show suggested prompts only when only the initial assistant message exists
  ...
  return (
    <div className="w-80 bg-white border-r border-gray-200 flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {/* Suggested prompts after initial message */}
          ...
          <div className="flex flex-col gap-2">
            {SUGGESTED_PROMPTS.map((prompt) => (
              <button
                ...
                className="px-3 py-2 text-xs text-left bg-gray-50 hover:bg-[#639922]/10 border border-gray-200 hover:border-[#639922] rounded-lg transition-colors text-gray-700"
              >
                &ldquo;{prompt}&rdquo;
              </button>
            ))}
        ...
        {isSearching && (
          <div className="flex justify-start">
            <div className="bg-blue-50 border border-blue-100 px-3 py-2 rounded-xl rounded-bl-sm flex items-center gap-2">
              ...
              <span className="text-xs text-blue-600">Researching district context...</span>
            </div>
          </div>
        )}
      ...
      <div className="border-t border-gray-200 p-3 flex-shrink-0">
        <form onSubmit={onSubmit} className="flex gap-2">
          <input
            ...
            disabled={isLoading}
            className="flex-1 px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#639922] focus:border-transparent bg-white disabled:bg-gray-50"
          />
          <button
            type="submit"
